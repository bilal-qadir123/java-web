...
<%
    String query = "SELECT AVG(calories) AS avg_calories FROM calories WHERE id=?";
    MyDb db = new MyDb();
...
<main class="table" id="calories_table">
    ...
    <section class="table__body">
        <table class="table">
            <thead>
            <tr>
                <th>Average Calories Consumed In The Recent Days</th>
            </tr>
            </thead>
            <tbody>
            <%
                if (rs.next()) {
                    hasData = true;
                    double avgCalories = rs.getDouble("avg_calories");
            %>
            <tr>
                <td><%= avgCalories %> Calories</td>
            </tr>
            <%
            } else {
            %>
            <tr>
                <td colspan="1">No average calories found for ID <%= id %></td>
            </tr>
            <%
                }
            } catch (Exception e) {
            %>
            <tr>
                <td colspan="1">Error: <%= e.getMessage() %></td>
            </tr>
            <%
                } finally {
